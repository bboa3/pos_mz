...
- Removed Arabic support from runtime:
  - Deleted `pos_next/translations/ar.csv`
...
- Consolidated Mozambican Portuguese translations in the `erpnext_mz` app:
  - `apps/erpnext_mz/erpnext_mz/translations/pt-MZ.csv` is the authoritative CSV served to POS MZ
...
- The authoritative locale is `pt-MZ` (Português (Moçambique)) provided by `erpnext_mz`.
- Ensure the `Language` DocType has `pt-MZ` enabled (via `erpnext_mz`’s setup or a site patch) so users/sites can select it.
...
1. Add/adjust strings in `apps/erpnext_mz/erpnext_mz/translations/pt-MZ.csv`
...
### Extending coverage
- `erpnext_mz/erpnext_mz/translations/pt-MZ.csv` now includes the high-frequency POS strings (payments, cart, items, shifts, receipts, errors).
...
- If you ever need a different regional Portuguese variant:
  ...
  - Duplicate `erpnext_mz/erpnext_mz/translations/pt-MZ.csv` to the new language file (or to another localization app)
...
If anything appears in English during usage, add the source string and Portuguese translation to `erpnext_mz/erpnext_mz/translations/pt-MZ.csv`, rebuild, and verify.
